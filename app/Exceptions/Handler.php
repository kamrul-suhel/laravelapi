...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Exception $exception)
    {
        ...
        if ($exception instanceof NotFoundHttpException) {
            return $this->errorResponse('This specified url can not found', 404);
        }

        if($exception instanceof MethodNotAllowedHttpException){
            return $this->errorResponse('This sepcified method for the request invalide', 405);
        }
        ...
        if($exception instanceof QueryException){
            $exceptionCode = $exception->errorInfo[1];

            if($exceptionCode === 1451){
                return $this->errorResponse('Can not remove permanently. It is related with any other resource', 409);
            }
        }
